...
let getLoopinGridData = async (siteUrl, currentContext, filterquery) => {
try {
let apiURL = siteUrl + `/_api/web/lists/GetByTitle('Template')/items?$Select=ID,TemplateName,TemplateDescription,MessageType,Created,Predefined&$filter=(TemplateType eq 'LoopIn')and(IsActive eq 1)${filterquery}`;
const response = await currentContext.spHttpClient.get(apiURL, SPHttpClient.configurations.v1);
const responsejson = await response.json()
Dispatcher.dispatch({ type: "LoopinGrid", value: responsejson.value });
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in getLoopinGridData");
}
};

let getLoopinArchiveData = async (siteUrl, currentContext, filterquery) => {
try {
let apiURL = siteUrl + `/_api/web/lists/GetByTitle('Template')/items?$Select=ID,TemplateName,TemplateDescription,MessageType,Created&$filter=(TemplateType eq 'LoopIn')and(IsActive eq -1)${filterquery}`;
const response = await currentContext.spHttpClient.get(apiURL, SPHttpClient.configurations.v1);
const responsejson = await response.json()
Dispatcher.dispatch({ type: "LoopinGrid", value: responsejson.value });
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in getLoopinArchiveData");
}
};
...
let apiURL4 = siteUrl + `/_api/web/lists/GetByTitle('TodoTemplate')/items?$select=ID,LoopInTemp/Id&$expand=LoopInTemp/LoopInTempID&$filter=(LoopInTemp/TemplateName eq '${DeleteTempalte}')`;
...
var arrindex;
var res = value.LoopInTemp.map((value1, index) => {
if (value1.Id == delindex) {
arrindex = index;
}
});
value.LoopInTemp.splice(arrindex, 1);
var LoopInArray = [];
var res = value.LoopInTemp.map((value2, index) => {
LoopInArray.push(value2.Id);
});
let apiURL5 = siteUrl + `/_api/web/lists/GetByTitle('TodoTemplate')/items('${value.ID}')`;
const data2: ISPHttpClientOptions = {
body: `{ LoopInTempId:[${LoopInArray}] }`,
headers: {
'accept': 'application/json;odata=nometadata',
"content-type": "application/json",
'odata-version': '',
...
let apiURL6 = siteUrl + `/_api/web/lists/GetByTitle('UserDetails')/items?$select=ID,LoopInTemp/Id&$expand=LoopInTemp/LoopInTempID&$filter=(LoopInTemp/TemplateName eq '${DeleteTempalte}')`;
...
Dispatcher.dispatch({ type: "successDeleteLoopin", value: response1.status });
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in postDeleteUser");
}
};
...
Dispatcher.dispatch({ type: "loopinArchive", value: response.status });
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in postArchiveUser");
}
};
...
const response = await currentContext.spHttpClient.post(apiURL, SPHttpClient.configurations.v1, data);
Dispatcher.dispatch({ type: "LoopinunArchive", value: response.status });
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in postunArchiveUser");
}
};

let getCreatedByNameLoopin = (siteUrl, currentContext) => {
try {
...
Dispatcher.dispatch({ type: "CreatedBylistLoopin", value: responseJSON.value });
});
});
}
catch (e) {
Exception.writeException(siteUrl, currentContext, e, "Loopin_Action in getCreatedByNameLoopin");
}
};

export { getLoopinGridData, postDeleteUser, postArchiveUser, postunArchiveUser, getLoopinArchiveData, getCreatedByNameLoopin }
